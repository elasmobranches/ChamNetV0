...
def run_inference(ckpt_path: str,   rgb_path: str,
    ...
    model: LightningESANetDepthOnly = LightningESANetDepthOnly.load_from_checkpoint(ckpt_path, map_location=device)
    model.eval()
    model.to(device)
...
def main():
    parser = argparse.ArgumentParser(description="Infer single RGB image with ESANet Depth-Only checkpoint")
